// niimath operations run a bundled native sidecar binary (a subprocess), which
// is desktop-only: iOS/Android forbid spawning executables, so this whole module
// is gated to desktop. niimath also ships as a WebAssembly build, so the
// mobile/web implementation should run that WASM build behind the same frontend
// interface rather than this native command.
...
use crate::{volumetric_server, NeuroVueState};

#[derive(Clone, Copy, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) enum NiimathOperation {
    Smooth,
    Threshold,
    UpperThreshold,
    Binarize,
...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct NiimathTaskRequest {
    source_path: String,
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct NiimathTaskResult {
    operation: NiimathOperation,
    source_path: String,
    output_path: String,
    volume_id: String,
